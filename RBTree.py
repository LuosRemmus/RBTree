...
class RBNode:
    ...
    def sibling(self):
        if self.isLeftChild():  # Если это левый дочерний элемент, вернуться к правому дочернему элементу
            # родительского узла
            return self.parent.right
        if self.isRightChild():  # Если это правый дочерний элемент, вернуться к левому дочернему элементу
            # родительского узла
    ...
    def predecessor(self, node):
        ...
        if node.left:  # Самый большой узел левого поддерева - предшественник
            ...
            while p.right:
                p = p.right
            return p
        # Нет левого поддерева, просто посмотрите вверх, если это правое поддерево родительского, то родительский
    ...
    def successor(self, node):
        ...
        if node.right:  # Наименьший узел правого поддерева является преемником
            ...
            while s.left:
                s = s.left
            return s
        # Правого поддерева нет, просто посмотрите вверх, если это левое поддерево родительского, то родительский
        # является преемником
